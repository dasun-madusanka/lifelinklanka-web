...
@Component({
  selector: 'app-shell',
  standalone: true,
  imports: [CommonModule, RouterLink, RouterOutlet, DatePipe, NavbarComponent, SidebarComponent, AlertBannerComponent, FooterComponent],
  template: `
    @if (auth.isLoggedIn()) {
      <!-- ======================================================== -->
      ...
      <div class="flex h-screen bg-slate-50 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
          ...
          <header class="h-16 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-30 shadow-xs shrink-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="relative">
                ...
                @if (showNotifications()) {
                  <div class="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in fade-in zoom-in duration-150">
              ...
              <div class="relative">
                ...
                @if (showUserMenu()) {
                  <div class="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-2 z-50 text-xs animate-in fade-in zoom-in duration-150">
      ...
      <div class="min-h-screen bg-slate-50 flex flex-col justify-between"
           [ngClass]="{
             'h-screen overflow-hidden': isLoginPage()
           }">
        <div class="flex-1 flex flex-col min-h-0"
             [ngClass]="{
               'h-full overflow-hidden': isLoginPage()
             }">
          <app-navbar />
          @if (!isAuthPage()) {
            <app-alert-banner />
...
export class ShellComponent implements OnInit {
  ...
  getMainClass(): string {
    if (this.isLoginPage()) {
      return 'flex-1 flex items-center justify-center p-2 sm:p-4 overflow-hidden';
    }
    if (this.isRegisterPage()) {
      return 'flex-1 overflow-y-auto px-4 py-4 sm:py-8 max-w-4xl mx-auto w-full';
    }
    if (this.isHomePage()) {
      return 'w-full p-0 m-0 flex-1';
    }
    return 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full flex-1';
  }
